...
`Agent/wiki/` held fifteen pages produced by the Stage-A librarian loop in the
...
fifteen Stage-A pages exist as rows. Until then an `--apply --prune` export would delete every
one of them, because no row accounts for any of them.

The order is: compilation ships, the existing pages are imported or recompiled through it,
*then* `Agent/wiki/` joins the owned set. Doing it in the other order is a data-loss bug with
a one-line diff, which is why the constant carries a comment saying so.
...
Whether wiki pages appear on the MCP tool surface. Compilation is REST-only here, matching how
the review flow shipped, and a `vault_compile_*` tool set is a separate decision about which
agent does the librarian's job.
