...
- ✅ `app/vault/passwords.py` — bcrypt, offloaded to a thread, and
  `VAULT_OPERATOR_PASSWORD_HASH` in config rather than a table. The hash is
  generated by `python -m scripts.hash_vault_operator_password`, and
  `vault-configuration.md` carries the full runbook
...
2. ~~**The operator credential.**~~ Done — `app/vault/passwords.py`,
   `VAULT_OPERATOR_PASSWORD_HASH`, and `scripts/hash_vault_operator_password.py`
   to generate the value. Config rather than a table: one secret, no lifecycle to
   model, rotation is `heroku config:set`. `bcrypt` is now listed in the
   extraction manifest as staying in both repos.
